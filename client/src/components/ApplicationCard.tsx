...
const ApplicationCard = ({application,userType,children}:ApplicationCardProps) => {
    const [imgSrc,setImgSrc] = useState(
        application.property.photosUrls?.[0] || "/placeholder.jpg"
    );

    const statusColor = application.status === "Approved"
    ? "bg-green-500" : application.status === "Denied" ? "bg-red-500" : "bg-yellow-500";
  ...
  return (
    <div className='border rounded-xl overflow-hidden shadow-sm bg-white mb-4'>
      <div className='flex felx-col gap-6 px-4 py-6 sm:flex-col lg:flex-row lg:items-center lg:justify-between'>
        {/** Property section */}
        ...
        <div className='flex flex-col lg:flex-row gap-5 w-full lg:w-auto'>
            ...
            onError={()=>setImgSrc("/placeholder.jgp")}/>
            <div className='flex flex-col justify-between'>
                <div>
                    ...
                    <div className='flex items-center mb-2'>
                        ...
                        <span> {`${application.property.location.city}`}</span>
                    </div>
                </div>
