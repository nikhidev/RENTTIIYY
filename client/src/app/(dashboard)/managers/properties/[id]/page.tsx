...
const PropertyTenants = () => {
  ...
  if (paymentLoading || leasesLoading || paymentLoading) return <Loading />;
  ...
  const getCurrentMonthPaymentStatus = (leaseId: number) => {
    ...
    const currentMonthPayment = payment?.find(
      (payment) =>
        ...
        new Date(payment.dueDate).getMonth() === currentData.getMonth() &&
        new Date(payment.dueDate).getFullYear() === currentData.getFullYear()
    );
      ...
      <div className="w-full  space-y-2">
        <div className="mt-8 bg-white rounded-xl shadow-md overflow-hidden p-6">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="text-3xl font-bold mb-1">Tenants Overview</h2>
              <p className="text-sm text-gray-500">
                Manage and View all tenats for this Property
              </p>
            </div>
            <div>
              <button
                className={`bg-whiet border-gray-300 text-gray-700 py-2 px-4 rounded-md flex items-center hover:bg-primary-700 hover:text-primary-50`}
              >
